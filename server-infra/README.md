...
### 4.2 Apacheのチューニング
Apacheでは並列処理を行うコア部分はMPM（Multi Processing Module）と呼ばれるモジュールとして実装されている。
代表的なMPMは以下の2つ
- prefork：あらかじめ複数のプロセスを生成してクライアントの接続に備えるマルチプロセスモデル。
- マルチスレッドとマルチプロセスのハイブリッド型

どちらにおいても共通しているのは、Apacheはクライアントからの1リクエストに対して、1プロセスもしくは1スレッドを割り当てて処理をする点である。

## 5章 省力運用
### 5.1 サービスの稼働監視
稼働監視はおもに以下の3つに分類される。
1. ホストやサービスの稼働状態といった死活状態の監視
pingによる応答が返ってくるか、TCPコネクションを張れるかなど、サービスの基本的なプロトコル処理ができるかどうかをチェックすることで、サービスが正しく動いているかを検出する。
2. ホストのCPU使用率やサービスの同時処理数などの負荷状態の監視
3. 一定期間でのサービス提供ができていた割合である稼働率の計測

上記のような監視や計測を可能にする代表的な監視ツールとして「Nagios」がある。

### 5.2 サーバリソースのモニタリング
モニタリングの目的は、サーバの状態を示すさまざまな指標、
- CPU使用率
- メモリ使用率
- ロードアベレージ
- ネットワークトラフィック　など
の変動を観察することである。
`Munin`、`Cacti`、`Ganglia`など、モニタリングのためのデータの収集とそのグラフ化をやってくれるツールがある。

### 5.3 サーバ管理の効率化
#### Puppet
Puppetは大規模環境での効率的なサーバ管理を実現するためのツールである。Puppetによって、各サーバにログインして手作業で行ってきた設定が、ほぼ自動的に反映させることができるようになる。これにより、
- 新規サーバの投入
- 既存サーバの設定変更
における各サーバへの設定の反映を省力化することができるようになる。

### 5.4 デーモンの稼働管理
#### daemontools
daemontoolsとは、デーモンプロセスの開始、終了、再起動、プロセスが落ちた場合の自動起動といったデーモンプロセスの管理を行うためのプログラム群です。
daemontoolsを使う大きなメリットは、
- プロセスが落ちた場合に、自動的に起動し直してくれる。
- 手軽にデーモンを作れる
などである。

### 5.5 ネットワークブートの活用
ネットワークブートとは、マシンがブートするために必要なデータやファイルをネットワークから取得してブートすること。
通常のブートでは、マシンの起動に必要なブートローダやOSのカーネルは、BIOSがローカルに接続されたハードディスクやCD-ROMなどの二次記憶装置から読み出す。これに対して、ネットワークブートでは、これらのファイルをネットワーク上のサーバから読み出す。
